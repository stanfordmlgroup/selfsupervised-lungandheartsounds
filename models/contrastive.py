...
class ContrastiveLearner(object):
    ...
    def fine_tune(self, n_splits, task, label_file, log_file, encoder=None, evaluator_type=None, learning_rate=0):
        ...
        if encoder is not None:
            ...
            for fold, (train_idx, test_idx) in enumerate(kf.split(df, df["y"])):
                ...
                train_X, train_y = get_scikit_loader(task, label_file, base_dir, "train", train_df, encoder)
                train_X = np.asarray(train_X)
                train_y = np.asarray(train_y)
                if evaluator_type == "linear":
                    evaluator = LogisticRegression(class_weight="balanced")
                ...
                print(
                    "Fold: {:03d}, Time: {:.3f} s\tFold Train Score: {:.7f}\n".format(
                        fold, elapsed_fold, fold_train_acc
                    )
                )
                with open(log_file, "a+") as log:
                    log.write(
                        "Fold: {:03d}, Time: {:.3f} s\tFold Train Acc: {:.7f}\n".format(
                            fold, elapsed_fold, fold_train_acc
                        )
                    )
    ...
    def test(self, task, label_file, log_file, encoder, evaluator_dir):
        ...
        with open(log_file, "w+") as out:
            if encoder is not None:
                encoder.eval()
                X, y = get_scikit_loader(task, label_file, base_dir, split="test", encoder=encoder)
                for i, model_weights in enumerate(glob(os.path.join(evaluator_dir, "evaluator_*.pkl"))):
                    ...
                    loss = loss.cuda()
                    ce = loss(torch.as_tensor(y_pred).float().to(self.device), torch.as_tensor(y).long().to(self.device))
...
def train_(task, base_dir, log_dir, evaluator, folds=5, train_prop=1):
    ...
    learner = ContrastiveLearner(dataset, num_epochs, batch_size, log_dir)
    learner.fine_tune(folds, task, label_file, log_file, evaluator, learning_rate)
