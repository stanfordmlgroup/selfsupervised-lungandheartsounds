...
def make_heart():
    ...
    fig = make_subplots(cols=2, horizontal_spacing=.17)
    ...
    fig.add_bar(x=data10['Runs'], y=data10['AUC'], error_y=dict(type='data', array=data10['ci_95']), row=1, col=1,
                marker=dict(color=color10))
    fig.add_bar(x=data100['Runs'], y=data100['AUC'], error_y=dict(type='data', array=data100['ci_95']), row=1, col=1,
                marker=dict(color=color100))
    ...
    color100 = ['rgb(120,120,120)' if ('Supervised' in name) else f'rgb({200 - 10 * i},0,0)' for i, name
                in enumerate(data100['Runs'])]
    fig.add_bar(x=data10['Runs'], y=data10['AUC'], error_y=dict(type='data', array=data10['ci_95']), row=1, col=2,
                marker=dict(color=color10))
    fig.add_bar(x=data100['Runs'], y=data100['AUC'], error_y=dict(type='data', array=data100['ci_95']), row=1, col=2,
                marker=dict(color=color100))
    ...
    fig.write_image('output/heart.png', scale=global_scale, width=1200, height=600)
...
def make_lung():
    ...
    # fig.show()
    fig.update_layout(showlegend=False)
    fig.write_image('output/lung.png', scale=global_scale, width=1200, height=600)
...
def make_metadata():
    ...
    fig = make_subplots(cols=2, horizontal_spacing=.2)
    metrics_df = pd.DataFrame(data, columns=['Runs', 'data', 'type', 'AUC', 'ci_95'])
    data = metrics_df[metrics_df['type'] == 'linear']
    data10 = data[data['data'] == "10%"]
    data100 = data[data['data'] == "100%"]
    ...
    # fig.show()
    fig.update_layout(showlegend=False)
    fig.write_image('output/demographics.png', scale=global_scale, width=1200, height=600)
